...
const logOfficeEvent = (event, details) => {
  ...
  console.log(`[Office Floor 3] ${timestamp}: ${event} - ${details}`);
};
...
app.post("/sensor-data", (req, res) => {
  ...
  res
    .status(200)
    .json({ message: "Office Floor 3 sensor data received successfully" });
});
...
app.post("/actuator-data", (req, res) => {
  ...
  res
    .status(200)
    .json({ message: "Office Floor 3 actuator data received successfully" });
});
...
const createOfficeAlert = (
  ...
  const alert = {
    alertId: `ALERT_OF3_${Date.now()}_${Math.random()
      .toString(36)
      .substr(2, 9)}`,
    alertType,
    severity,
...
// Start server
app.listen(port, () => {
  console.log(`Office Floor 3 Web Server running on http://localhost:${port}`);
  console.log("=== Office Floor 3 MONITORING SYSTEM ===");
  ...
  logOfficeEvent("SYSTEM_STARTUP", "Office Floor 3 Web Server initialized");
});

// Graceful shutdown
process.on("SIGINT", () => {
  logOfficeEvent("SYSTEM_SHUTDOWN", "Office Floor 3 Web Server shutting down");
  console.log("\nShutting down Office Floor 3 web server...");
  process.exit(0);
});
