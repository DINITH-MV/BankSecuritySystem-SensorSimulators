...
const logOfficeEvent = (event, details) => {
  ...
  console.log(`[Office Floor 29] ${timestamp}: ${event} - ${details}`);
};
...
app.post("/sensor-data", (req, res) => {
  ...
  res
    .status(200)
    .json({ message: "Office Floor 29 sensor data received successfully" });
});
...
app.post("/actuator-data", (req, res) => {
  ...
  res
    .status(200)
    .json({ message: "Office Floor 29 actuator data received successfully" });
});
...
const createOfficeAlert = (
  ...
  const alert = {
    alertId: `ALERT_OF29_${Date.now()}_${Math.random()
      .toString(36)
      .substr(2, 9)}`,
    alertType,
    severity,
...
// Start server
app.listen(port, () => {
  console.log(`Office Floor 29 Web Server running on http://localhost:${port}`);
  console.log("=== Office Floor 29 MONITORING SYSTEM ===");
  ...
  logOfficeEvent("SYSTEM_STARTUP", "Office Floor 29 Web Server initialized");
});

// Graceful shutdown
process.on("SIGINT", () => {
  logOfficeEvent("SYSTEM_SHUTDOWN", "Office Floor 29 Web Server shutting down");
  console.log("\nShutting down Office Floor 29 web server...");
  process.exit(0);
});
