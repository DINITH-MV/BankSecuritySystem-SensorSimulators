...
// Import sensor nodes
import "../zones/office_floor_8/sensors/AXIS_XFQ1656_camera_node.js";
import "../zones/office_floor_8/sensors/Bosch_PIR_motion_detector_node.js";
import "../zones/office_floor_8/sensors/HID_MiniProx_proxpoint_reader.js";
import "../zones/office_floor_8/sensors/Honeywell_DT8016_motion_detector_node.js";

// Import actuator nodes
import "../zones/office_floor_8/actuators/E2S_HMA121_Hootronic_siren.js";
import "../zones/office_floor_8/actuators/Werma_D62_LED_beacon_node.js";
...
// Global data storage for Office Floor 8
global.officeFloor8SensorData = {};
global.officeFloor8ActuatorData = {};

// Command storage for actuator control
let sirenCommands = [];
let beaconCommands = [];

// Office Floor 8 Sensor data endpoint
app.get("/sensor-data", (req, res) => {
  try {
    const sensorDataArray = [];

    // Convert global sensor data to array format
    Object.keys(global.officeFloor8SensorData).forEach((sensorId) => {
      const sensorData = global.officeFloor8SensorData[sensorId];
      if (sensorData) {
    ...
    console.log(
      `Office Floor 8: Serving ${sensorDataArray.length} sensor readings`
    );
...
app.post("/sensor-data", (req, res) => {
  try {
    ...
    global.officeFloor8SensorData[sensorData.sensorId] = {
      ...sensorData,
    ...
    console.log(
      `Office Floor 8: Received data from ${sensorData.sensorType} (${sensorData.sensorId})`
    );
...
// Office Floor 8 Actuator data endpoint
app.get("/actuator-data", (req, res) => {
  try {
    const actuatorDataArray = [];

    // Convert global actuator data to array format
    Object.keys(global.officeFloor8ActuatorData).forEach((actuatorId) => {
      const actuatorData = global.officeFloor8ActuatorData[actuatorId];
      if (actuatorData) {
    ...
    console.log(
      `Office Floor 8: Serving ${actuatorDataArray.length} actuator states`
    );
...
app.post("/actuator-data", (req, res) => {
  try {
    ...
    global.officeFloor8ActuatorData[actuatorId] = {
      ...actuatorData,
      receivedAt: new Date().toISOString(),
      zone: "office_floor_8",
    };
    ...
    console.log(
      `Office Floor 8: Received data from ${
...
app.post("/activate-actuator", (req, res) => {
  try {
    ...
    console.log(
      `Office Floor 8: Activating ${actuatorType} with action: ${action}`
    );

    // Find matching actuators by type
    const matchingActuators = [];
    Object.keys(global.officeFloor8ActuatorData).forEach((actuatorId) => {
      const actuator = global.officeFloor8ActuatorData[actuatorId];
    ...
    matchingActuators.forEach(({ id, data }) => {
      ...
      // Update global state
      global.officeFloor8ActuatorData[id] = updatedState;
      ...
      console.log(
        `Office Floor 8: Successfully activated ${actuatorType} (${id})`
      );
    });
...
app.post("/manual-command", (req, res) => {
  try {
    ...
    console.log(`Office Floor 8: Manual command for ${actuatorId}: ${action}`);

    // Check if actuator exists
    if (!global.officeFloor8ActuatorData[actuatorId]) {
      return res.status(404).json({
    ...
    // Update global state
    global.officeFloor8ActuatorData[actuatorId] = updatedState;
...
app.post("/siren-commands/:zone", (req, res) => {
  ...
  if (zone === "office_floor_8") {
    ...
    console.log(
      `Office Floor 8: SIREN_COMMAND_RECEIVED - ${
...
app.post("/beacon-commands/:zone", (req, res) => {
  ...
  if (zone === "office_floor_8") {
    ...
    console.log(
      `Office Floor 8: BEACON_COMMAND_RECEIVED - ${
        commandData.active ? "ACTIVATE" : "DEACTIVATE"
...
app.get("/zone-status", (req, res) => {
  try {
    const sensorCount = Object.keys(global.officeFloor8SensorData).length;
    const actuatorCount = Object.keys(global.officeFloor8ActuatorData).length;

    const activeSensors = Object.values(global.officeFloor8SensorData).filter(
...
app.get("/", (req, res) => {
  res.json({
    message: "Office Floor 8 Security System Web Server",
    zone: "office_floor_8",
    port: PORT,
    timestamp: new Date().toISOString(),
    endpoints: {
    ...
    zone_info: {
      ...
      type: "office",
      description:
        "Office Floor 8 - Standard office environment security monitoring",
    },
  });
});
...
app.use((err, req, res, next) => {
  console.error("Office Floor 8 Server Error:", err.stack);
  res.status(500).json({
    error: "Internal server error",
    zone: "office_floor_8",
    message: err.message,
...
// Start the server
app.listen(PORT, () => {
  console.log(`\n=== Office Floor 8 Security System Web Server ===`);
  console.log(`Server running on http://localhost:${PORT}`);
  console.log(`Zone: office_floor_8`);
