...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF9_001", location: "Office Floor 9 Main Entrance" },
  {
    readerId: "ACCESS_OF9_002",
    location: "Office Floor 9 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF9_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 9..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 9 Card Reader Nodes...");
