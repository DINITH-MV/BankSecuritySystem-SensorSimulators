...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF21_001", location: "Office Floor 21 Main Entrance" },
  {
    readerId: "ACCESS_OF21_002",
    location: "Office Floor 21 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF21_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 21..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 21 Card Reader Nodes...");
