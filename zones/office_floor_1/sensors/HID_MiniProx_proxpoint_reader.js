...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF1_001", location: "Office Floor 1 Main Entrance" },
  {
    readerId: "ACCESS_OF1_002",
    location: "Office Floor 1 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF1_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 1..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 1 Card Reader Nodes...");
