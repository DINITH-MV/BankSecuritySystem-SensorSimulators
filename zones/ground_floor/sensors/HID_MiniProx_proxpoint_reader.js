...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_001", location: "Main Entrance Door 1" },
  { readerId: "ACCESS_002", location: "Main Entrance Door 2" },
  { readerId: "ACCESS_003", location: "Reception Desk Entry" },
  { readerId: "ACCESS_004", location: "Turnstile 1" },
  { readerId: "ACCESS_005", location: "Turnstile 2" },
  { readerId: "ACCESS_006", location: "Service Entrance" },
  { readerId: "ACCESS_007", location: "Emergency Exit" },
  { readerId: "ACCESS_008", location: "Elevator Access Point" },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for ground floor entrances..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Ground Floor Card Reader Nodes...");
