...
class GroundFloorSensorController {
  constructor() {
    this.zone = "ground_floor";
    this.sensors = {};
    ...
    const mqttOptions = {
      ...
      clientId: `ground_floor_${Math.random().toString(16).substr(2, 8)}`,
      clean: true,
      connectTimeout: 4000,
  ...
  setupMQTT() {
    this.mqttClient.on("connect", () => {
      console.log("Ground Floor Sensor Controller connected to MQTT");
  ...
  async fetchSensorData() {
    try {
      ...
      if (error.response) {
        ...
        console.error("Make sure the web server is running on port 3001");
      } else {
  ...
  checkForAlerts(sensorId, sensorType, data) {
    ...
    if (sensorType === "motion_detectors" && data.motionDetected) {
      alert = {
    ...
    // Access reader alerts
    if (sensorType === "access_readers" && data.accessAttempt) {
    ...
    // Camera alerts
    if (sensorType === "cameras" && data.motionInFrame) {
      alert = {
        type: "CAMERA_MOTION_DETECTED",
        severity: "low",
        data: {
          location: data.location,
          timestamp: data.timestamp,
...
// Start the Ground Floor Sensor Controller
const controller = new GroundFloorSensorController();
...
export default GroundFloorSensorController;
