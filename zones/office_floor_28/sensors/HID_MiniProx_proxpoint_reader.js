...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF28_001", location: "Office Floor 28 Main Entrance" },
  {
    readerId: "ACCESS_OF28_002",
    location: "Office Floor 28 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF28_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 28..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 28 Card Reader Nodes...");
