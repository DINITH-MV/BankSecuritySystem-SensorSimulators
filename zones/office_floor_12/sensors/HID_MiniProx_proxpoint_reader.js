...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF12_001", location: "Office Floor 12 Main Entrance" },
  {
    readerId: "ACCESS_OF12_002",
    location: "Office Floor 12 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF12_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 12..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 12 Card Reader Nodes...");
