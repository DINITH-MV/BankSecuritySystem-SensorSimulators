...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF25_001", location: "Office Floor 25 Main Entrance" },
  {
    readerId: "ACCESS_OF25_002",
    location: "Office Floor 25 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF25_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 25..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 25 Card Reader Nodes...");
