...
const MOTION_CONFIGS = [
  { sensorId: "MOTION_H5_015", location: "High Security Entrance Foyer" },
...
async function sendDataToServer(sensorData) {
  try {
    ...
    if (sensorData.data.motionDetected) {
      console.log(
        `HIGH SECURITY MOTION DETECTED - ${sensorData.sensorType}: Level ${sensorData.data.motionLevel}% at ${sensorData.location}`
      );
    } else {
      console.log(
        `${sensorData.sensorType}: High security area secure at ${sensorData.location}`
      );
    }
  } catch (error) {
    console.error(
...
console.log(
  "Starting Honeywell DT8016 DualTech Motion Detector for high security floor 5..."
);
...
process.on("SIGINT", () => {
  console.log(
    "\nShutting down High Security Floor 5 Honeywell Motion Detector Nodes..."
  );
