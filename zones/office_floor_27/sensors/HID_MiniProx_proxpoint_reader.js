...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF27_001", location: "Office Floor 27 Main Entrance" },
  {
    readerId: "ACCESS_OF27_002",
    location: "Office Floor 27 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF27_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 27..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 27 Card Reader Nodes...");
