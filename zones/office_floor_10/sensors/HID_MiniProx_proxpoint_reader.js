...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF10_001", location: "Office Floor 10 Main Entrance" },
  {
    readerId: "ACCESS_OF10_002",
    location: "Office Floor 10 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF10_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 10..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 10 Card Reader Nodes...");
