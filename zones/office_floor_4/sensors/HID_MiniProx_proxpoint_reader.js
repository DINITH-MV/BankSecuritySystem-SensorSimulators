...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF4_001", location: "Office Floor 4 Main Entrance" },
  {
    readerId: "ACCESS_OF4_002",
    location: "Office Floor 4 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF4_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 4..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 4 Card Reader Nodes...");
