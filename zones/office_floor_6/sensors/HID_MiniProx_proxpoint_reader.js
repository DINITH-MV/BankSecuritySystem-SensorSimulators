...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF6_001", location: "Office Floor 6 Main Entrance" },
  {
    readerId: "ACCESS_OF6_002",
    location: "Office Floor 6 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF6_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 6..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 6 Card Reader Nodes...");
