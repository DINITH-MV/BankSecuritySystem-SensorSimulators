...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF23_001", location: "Office Floor 23 Main Entrance" },
  {
    readerId: "ACCESS_OF23_002",
    location: "Office Floor 23 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF23_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 23..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 23 Card Reader Nodes...");
