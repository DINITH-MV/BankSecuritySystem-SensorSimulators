...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF29_001", location: "Office Floor 29 Main Entrance" },
  {
    readerId: "ACCESS_OF29_002",
    location: "Office Floor 29 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF29_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 29..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 29 Card Reader Nodes...");
