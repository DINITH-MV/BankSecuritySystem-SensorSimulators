...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF24_001", location: "Office Floor 24 Main Entrance" },
  {
    readerId: "ACCESS_OF24_002",
    location: "Office Floor 24 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF24_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 24..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 24 Card Reader Nodes...");
