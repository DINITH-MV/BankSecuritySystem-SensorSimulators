...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF5_001", location: "Office Floor 5 Main Entrance" },
  {
    readerId: "ACCESS_OF5_002",
    location: "Office Floor 5 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF5_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 5..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 5 Card Reader Nodes...");
