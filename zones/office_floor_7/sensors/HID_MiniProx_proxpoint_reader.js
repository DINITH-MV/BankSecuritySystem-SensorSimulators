...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF7_001", location: "Office Floor 7 Main Entrance" },
  {
    readerId: "ACCESS_OF7_002",
    location: "Office Floor 7 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF7_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 7..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 7 Card Reader Nodes...");
