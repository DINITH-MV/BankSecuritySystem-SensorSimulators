...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF30_001", location: "Office Floor 30 Main Entrance" },
  {
    readerId: "ACCESS_OF30_002",
    location: "Office Floor 30 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF30_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 30..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 30 Card Reader Nodes...");
