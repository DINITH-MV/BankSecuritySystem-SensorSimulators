...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF8_001", location: "Office Floor 8 Main Entrance" },
  {
    readerId: "ACCESS_OF8_002",
    location: "Office Floor 8 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF8_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 8..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 8 Card Reader Nodes...");
