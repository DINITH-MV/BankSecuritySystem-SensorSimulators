...
class OfficeFloor8SensorController {
  constructor() {
    ...
    const mqttOptions = {
      ...
      clientId: `office_floor_8_${Math.random().toString(16).substr(2, 8)}`,
      clean: true,
      connectTimeout: 4000,
  ...
  setupMQTT() {
    this.mqttClient.on("connect", () => {
      console.log("Office Floor 8 Sensor Controller connected to MQTT");
  ...
  triggerAlert(sensorId, alert) {
    ...
    console.log(
      `Office Floor 8 ALERT TRIGGERED: ${alert.type} - ${alert.severity}`
    );
...
// Start the Office Floor 8 Sensor Controller
const controller = new OfficeFloor8SensorController();
...
export default OfficeFloor8SensorController;
