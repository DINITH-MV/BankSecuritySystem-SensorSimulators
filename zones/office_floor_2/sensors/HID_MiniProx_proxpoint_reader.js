...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF2_001", location: "Office Floor 2 Main Entrance" },
  {
    readerId: "ACCESS_OF2_002",
    location: "Office Floor 2 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF2_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for office floor 2..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 2 Card Reader Nodes...");
