...
class OfficeFloor2SensorController {
  constructor() {
    ...
    const mqttOptions = {
      ...
      clientId: `office_floor_2_${Math.random().toString(16).substr(2, 8)}`,
      clean: true,
      connectTimeout: 4000,
  ...
  setupMQTT() {
    this.mqttClient.on("connect", () => {
      console.log("Office Floor 2 Sensor Controller connected to MQTT");
  ...
  triggerAlert(sensorId, alert) {
    ...
    console.log(
      `OFFICE FLOOR 2 ALERT TRIGGERED: ${alert.type} - ${alert.severity}`
    );
    ...
    this.mqttClient.publish(
      ...
      (err) => {
        if (err) {
          console.error(`Error publishing alert to ${topic}:`, err.message);
        } else {
...
// Start the Office Floor 2 Sensor Controller
const controller = new OfficeFloor2SensorController();
...
export default OfficeFloor2SensorController;
