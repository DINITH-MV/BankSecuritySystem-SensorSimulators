...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF3_001", location: "Office Floor 18 Main Entrance" },
  {
    readerId: "ACCESS_OF3_002",
    location: "Office Floor 18 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF3_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 18..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 18 Card Reader Nodes...");
