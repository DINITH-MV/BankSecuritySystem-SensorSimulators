...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF11_001", location: "Office Floor 11 Main Entrance" },
  {
    readerId: "ACCESS_OF11_002",
    location: "Office Floor 11 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF11_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 11..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 11 Card Reader Nodes...");
