...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF16_001", location: "Office Floor 16 Main Entrance" },
  {
    readerId: "ACCESS_OF16_002",
    location: "Office Floor 16 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF16_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 16..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 16 Card Reader Nodes...");
