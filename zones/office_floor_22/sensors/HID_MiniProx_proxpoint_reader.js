...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF22_001", location: "Office Floor 22 Main Entrance" },
  {
    readerId: "ACCESS_OF22_002",
    location: "Office Floor 22 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF22_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for Office Floor 22..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down Office Floor 22 Card Reader Nodes...");
