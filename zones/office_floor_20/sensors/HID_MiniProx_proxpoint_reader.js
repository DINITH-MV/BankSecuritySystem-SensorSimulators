...
const CARD_READER_CONFIGS = [
  { readerId: "ACCESS_OF20_001", location: "office floor 20 Main Entrance" },
  {
    readerId: "ACCESS_OF20_002",
    location: "office floor 20 Secure Access Point",
  },
];
...
function generateCardReaderData(config) {
  ...
  const cardId = cardPresented
    ? `CARD_OF20_${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    : null;
...
console.log(
  "Starting HID ProxPoint Card Reader monitoring for office floor 20..."
);
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\nShutting down office floor 20 Card Reader Nodes...");
